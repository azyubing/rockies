function orderDatePicker(){
	    	...
	    	if(checkForm(form)){
	    		...
	    		$(".counterItem").each(function(){
	    			...
	    			if($this.find(".numField").val() > 0){
		    			...
		    			obj.id = info.id;
		    			obj.tname = info.room_type_name;
		    			obj.num = parseInt($this.find(".numField").val());
		    			obj.price = info.room_price;
	...
	self.counterAdd = function(dom){
	    ...
	    var max = dom.data("info").max_cnt;
		if(max && max <= num){
			modal_alert("最多可预定"+max+"个")
			return;
		}
	    fieldElm.val(++num);
	    self.feeTotal();
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data.status == 'success'){
						...
						var render = template.compile(tmpl_counterHotel);
