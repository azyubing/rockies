...
<div class="page-content">
	<div class="page-content-area">
        <div class="row">
			<div class="col-md-12">
				<h3 class="header smaller lighter blue">套餐价格列表</h3>
...
window.operateEvents = {
        'click #detailBtn': function (e, value, row, index) {
        	window.location.href='<%=basePath%>setpack/toUpdatePackagePriceAdmin.html?menu=${menu}&pid='+row.pid+'&id='+row.id;
        },
        'click #deleteBtn': function (e, value, row, index) {
			$.messager.confirm("提示", "是否确认删除:"+name, function() {
				...
				$.ajax({
					type:"POST",
					url:"<%=basePath%>setpack/deletePackagePriceAdmin.html",
					data:{'pid': row.pid,'id': row.id },
					dataType: "html",
					success: function(json){
						$('#tableInfo').bootstrapTable('refresh',{url: "<%=basePath%>setpack/packagePriceAdminSearchList.html"});
...
function doSearch(){
    		...
    		var url = "<%=basePath%>setpack/packagePriceAdminSearchList.html";
	...
	$(function() { 
		...
		$("#addBtn").click(function(){
			window.location.href='<%=basePath%>setpack/toAddPackagePriceAdmin.html?menu=${menu}&pid=${row.pid}';
		});
