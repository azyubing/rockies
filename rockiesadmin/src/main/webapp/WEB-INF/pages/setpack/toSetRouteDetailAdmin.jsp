...
<div class="page-content">
	<div class="page-content-area">
        <div class="row">
			<div class="col-md-12">
				<h3 class="header smaller lighter blue">套餐行程详细列表</h3>
						...
						<div class="widget-main">
							<div class="row">
								<div class="col-md-4">
									...
									<input id="pday" value="${row.pday }" type="hidden" />
								</div>
								<div class="col-md-8">
...
window.operateEvents = {
        'click #detailBtn': function (e, value, row, index) {
        	window.location.href='<%=basePath%>setpack/toUpdateRouteDetailAdmin.html?menu=${menu}&pid='+row.pid+'&pday='+row.pday+'&starttime='+row.starttime;
        },
        'click #deleteBtn': function (e, value, row, index) {
			$.messager.confirm("提示", "是否确认删除:"+row.title, function() {
				...
				$.ajax({
					type:"POST",
					url:"<%=basePath%>setpack/deleteRouteDetailAdmin.html",
					data:{'pid': row.pid,'pday': row.pday,'starttime': row.starttime },
					dataType: "html",
					success: function(json){
						$('#tableInfo').bootstrapTable('refresh',{url: "<%=basePath%>setpack/routeDetailAdminSearchList.html"});
...
function doSearch(){
    		...
    		var url = "<%=basePath%>setpack/routeDetailAdminSearchList.html";
	...
	$(function() { 
		...
		$("#addBtn").click(function(){
			window.location.href='<%=basePath%>setpack/toAddRouteDetailAdmin.html?menu=${menu}&pid=${row.pid}&pday=${row.pday}';
		});
