...
<div class="page-content">
	<div class="page-content-area">
        <div class="row">
			<div class="col-md-12">
				<h3 class="header smaller lighter blue">套餐管理</h3>
...
function operateFormatter(value, row, index) { 
	return [
			...
			'</a>',
	        '<a class="edit margin-left-20" href="javascript:void(0)" title="add room" id="toSetPackagePriceAdmin">',
	            '<i class="glyphicon glyphicon-edit"></i> 查看价格',
	        '</a>',
	        '<a class="edit margin-left-20" href="javascript:void(0)" title="add room" id="toSetRouteDayAdmin">',
	            '<i class="glyphicon glyphicon-edit"></i> 套餐行程',
	        '</a>',
...
window.operateEvents = {
        ...
        'click #toSetPackagePriceAdmin': function (e, value, row, index) {
			window.location.href='<%=basePath%>setpack/toSetPackagePriceAdmin.html?menu=${menu}&pid='+row.pid;
        },
        'click #toSetRouteDayAdmin': function (e, value, row, index) {
			window.location.href='<%=basePath%>setpack/toSetRouteDayAdmin.html?menu=${menu}&pid='+row.pid;
        },
        'click #detailBtn': function (e, value, row, index) {
        	console.log()
        	window.location.href='<%=basePath%>setpack/toUpdatePackageAdmin.html?menu=${menu}&pid='+row.pid;
        },
        'click #deleteBtn': function (e, value, row, index) {
			$.messager.confirm("提示", "是否确认删除:"+row.pname, function() {
				...
				$.ajax({
					type:"POST",
					url:"<%=basePath%>setpack/deletePackageAdmin.html",
					data:{'pid': row.pid},
					dataType: "html",
					success: function(json){
						$('#tableInfo').bootstrapTable('refresh',{url: "<%=basePath%>setpack/packageAdminSearchList.html"});
...
function doSearch(){
    		...
    		var url = "<%=basePath%>setpack/packageAdminSearchList.html";
	...
	$(function() { 
		...
		$("#addBtn").click(function(){
			window.location.href = '<%=basePath%>setpack/toAddPackageAdmin.html?menu=${menu}';
		});
