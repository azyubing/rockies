...
<div class="page-content">
	<div class="page-content-area">
        <div class="row">
			<div class="col-md-12">
				<h3 class="header smaller lighter blue">交通管理</h3>
...
function operateFormatter(value, row, index) { 
	...
	if(row.traffic_type == 0){
		return [
				...
				'</a>',
			    '<a class="edit margin-left-20" href="javascript:void(0)" title="traffic chartered price" id="toSetTrafficCharteredPriceAdmin">',
		...
		return [
				...
				'<a class="edit margin-left-20" href="javascript:void(0)" title="Set Price" id="setExtraCostBtn">',
					'<i class="glyphicon glyphicon-edit"></i> 加价设定',
				'</a>',
			    '<a class="edit margin-left-20" href="javascript:void(0)" title="traffic regular price" id="toSetTrafficRegularPriceAdmin">',
			        '<i class="glyphicon glyphicon-edit"></i> 查看价格',
			    '</a>',
			    '<a class="edit margin-left-20" href="javascript:void(0)" title="update" id="detailBtn">',
			        '<i class="glyphicon glyphicon-edit"></i> 修改',
...
window.operateEvents = {
        ...
        'click #toSetTrafficCharteredPriceAdmin': function (e, value, row, index) {
			window.location.href='<%=basePath%>traffic/toSetTrafficCharteredPriceAdmin.html?menu=${menu}&pid='+row.pid;
        },
        'click #toSetTrafficRegularPriceAdmin': function (e, value, row, index) {
			window.location.href='<%=basePath%>traffic/toSetTrafficRegularPriceAdmin.html?menu=${menu}&pid='+row.pid;
        },
        'click #detailBtn': function (e, value, row, index) {
        	window.location.href='<%=basePath%>traffic/toUpdateTrafficAdmin.html?menu=${menu}&pid='+row.pid;
        },
        'click #deleteBtn': function (e, value, row, index) {
			$.messager.confirm("提示", "是否确认删除:"+row.pname, function() {
				...
				$.ajax({
					type:"POST",
					url:"<%=basePath%>traffic/deleteTrafficAdmin.html",
					data:{'pid': row.pid},
					dataType: "html",
					success: function(json){
						$('#tableInfo').bootstrapTable('refresh',{url: "<%=basePath%>traffic/trafficAdminSearchList.html"});
...
function doSearch(){
    		...
    		var url = "<%=basePath%>traffic/trafficAdminSearchList.html";
	...
	$(function() { 
		...
		$("#addBtn").click(function(){
			window.location.href = '<%=basePath%>traffic/toAddTrafficAdmin.html?menu=${menu}';
		});
