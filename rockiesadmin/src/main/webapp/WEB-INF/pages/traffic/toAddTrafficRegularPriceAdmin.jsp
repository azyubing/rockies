				...
				<div class="widget-box">
					...
					<div class="widget-body">
						<div class="widget-main">
							<form action="<%=basePath%>traffic/addTrafficRegularPriceAdmin.html" id="addForm" name="addForm" method="post">
								...
								<div class="row">
									<div class="col-md-6">
										...
										<input id="departtime" name="departtime" onblur="checkStartTimeRepeat()" type="text" class="form-control " placeholder="请输入内容" style="display: inline-block; width: 50%;"/>
										<span style="color: red;">*输入的时间格式如:09:30</span>
									</div>
									<div class="col-md-6">
										<label class=""> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;班车排序： </label>
							...
							<form action="<%=basePath%>img/uploadFileList.html" id="attachedImgFileForm" method="post" enctype="multipart/form-data" >	
								<div class="row">
									<div class="col-md-12 attachedImg">
										<label class="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;交通班车附属图（单选）<span style="color:red">(需要上传宽高比列为2:1的图片，用于详细页面滚动显示)</span>： </label>
...
<script type="text/javascript">
	...
	function checkStartTimeRepeat(){
		...
		$.ajax({
			type:"GET",
			url:"<%=basePath%>traffic/checkStartTimeRegularPriceRepeat.html",
			data:{'pid': $("#pid").val(),'departtime': $("#departtime").val() },
			dataType: "html",
			...
			$("#attachedImgFileForm").ajaxSubmit({
                ...
                success: function(json){
                	...
                	if(errcode == 0){
                    	...
                    	$("#attachedImgMsg").text("交通班车附属图上传成功");
                    	attachedImgPreview(data.join(","));
                        
                	}else{
                		$("#attachedImgMsg").text("交通班车附属图上传失败:"+result.errmsg);
                	}
                    shCircleLoaderDestroy();
                } 
            });
...
function attachedImgPreview(json){
	...
	$(attachedImgPreviewArray).each(function(index){
		li_img_Str += 
			'<li>'+
				'<a href="'+attachedImgPreviewArray[index]+'" id="attachedImgHref" title="交通班车附属图'+(index+1)+'预览" data-rel="colorbox">'+
					'<img width="300" height="150" alt="300x150" id="attachedImgReviewImage" src="'+attachedImgPreviewArray[index]+'" />'+
					'<div class="text">'+
						'<div class="inner">交通班车附属图'+(index+1)+'预览</div>'+
					'</div>'+
				'</a>'+
			'</li>'
	});
