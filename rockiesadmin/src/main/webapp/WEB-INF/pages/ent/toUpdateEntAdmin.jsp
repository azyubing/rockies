...
<div class="breadcrumbs breadcrumbs-fixed" id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li>
				<span style="color: #669FC7;">${pTypeName }修改</span>
			</li>
		</ul>
...
<div class="page-content">
	<div class="page-content-area">
        <div class="row">
			<div class="col-md-12">
				<h3 class="header smaller lighter blue">${pTypeName }修改</h3>
				<div class="widget-box">
					
					<div class="widget-body">
						<div class="widget-main">
							<form action="<%=basePath%>ent/updateEntAdmin.html" id="updateForm" name="updateForm" method="post">
								<%-- 基础信息 --%>
								...
								<div class="row">
									<div class="col-md-6">
										...
										<select id="enttype" name="enttype" class="form-control"  style="display: inline-block; width: 50%;"/>  
											<option value="" <c:if test="${row.enttype eq '' }"> selected="selected"</c:if> >-请选择-</option>
											<option value="0" <c:if test="${row.enttype eq '0' }"> selected="selected"</c:if> >门票</option>
											<option value="1" <c:if test="${row.enttype eq '1' }"> selected="selected"</c:if> >观光</option>
						                </select>
									</div>
									<div class="col-md-6">
										<label class=""> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;服务类型： </label>
										<select id="servicetype" name="servicetype" class="form-control"  style="display: inline-block; width: 50%;"/>  
											<option value="" <c:if test="${row.servicetype eq '' }"> selected="selected"</c:if> >-请选择-</option>
											<option value="0" <c:if test="${row.servicetype eq '0' }"> selected="selected"</c:if> >散团</option>
											<option value="1" <c:if test="${row.servicetype eq '1' }"> selected="selected"</c:if> >独立包团</option>
						                </select>
									</div>
								</div>
								<br>
								<div class="row">
									<div class="col-md-6">
										...
										<select id="language" name="language" class="form-control"  style="display: inline-block; width: 50%;"/>  
											<option value="" <c:if test="${row.language eq '' }"> selected="selected"</c:if> >-请选择-</option>
											<option value="0" <c:if test="${row.language eq '0' }"> selected="selected"</c:if> >中文</option>
											<option value="1" <c:if test="${row.language eq '1' }"> selected="selected"</c:if> >英文</option>
											<option value="2" <c:if test="${row.language eq '2' }"> selected="selected"</c:if> >任意</option>
						                </select>
									</div>
								</div>
								<br>
