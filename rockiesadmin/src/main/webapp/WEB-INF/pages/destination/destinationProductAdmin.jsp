	...
	$(function() { 
		...
		$("#addBtn").click(function(){
            ...
            $.messager.confirm("提示", "是否确认保存:"+pnames, function() {
				...
				$.ajax({
		            type:'post',        
		            url:'<%=basePath%>destination/checkDestinationProduct.html',    
		            data:{"ids":ids.toString(),"destinationId":'${destinationVO.id}',"menu":'${menu}'},    
		            cache:false,    
		            dataType:'text',    
		            success:function(data){
		            	if(data=='success'){
		            		$.ajax({
		    		            type:'post',        
		    		            url:'<%=basePath%>destination/saveDestinationProduct.html', 
		    		            data:{"ids":ids.toString(),"destinationId":'${destinationVO.id}',"menu":'${menu}'},    
		    		            cache:false,    
		    		            dataType:'text',    
		    		            success:function(data){
		    		            	if(data=='success'){
		    		            		alert("添加成功");
		    		            		window.location.href= '<%=basePath%>destination/destinationSetProduct.html?menu=${menu}&id=${destinationVO.id}';
		    		            	}
		    		            }    
		    		        });
		            	}else{
